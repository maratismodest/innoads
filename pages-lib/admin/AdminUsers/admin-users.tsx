  ...
  return (
    <div className="grid grid-cols-1 gap-2">
      <form className="flex items-center gap-4" onSubmit={handleSubmit(onSubmit)}>
        <input {...register('username')} className={inputStyles()} placeholder="@username" />
        <button className={buttonStyles({ size: 'medium' })} type="submit">
          Поиск
        </button>
      </form>
      {usersLoading && <Spinner />}
      {usersError && <h1>Ошибка при получении данных о пользователях</h1>}
      {!usersLoading && !usersError && items.length > 0 && (
        <VirtualList
          items={items}
          itemHeight={38}
          windowHeight={800}
